...
const Affix = defineComponent({
  ...
  setup (props, { slots, emit }) {
    ...
    const handleScroll = () => {
      ...
      const elHeight = (affixRoot.value as HTMLElement).getElementsByTagName('div')[0].offsetHeight;
      // Fixed Top
      if ((elOffset.top - offsetTop.value) < scrollTop && offsetType.value === 'top' && !affix.value) {
          ...
          emit('on-change', true);
      } else if ((elOffset.top - offsetTop.value) > scrollTop && offsetType.value == 'top' && affix.value) {
          slot.value = false;
          state.slotStyle = {};
          affix.value = false;
          state.styles = {};
      ...
      // Fixed Bottom
      if ((elOffset.top + offsetBottom.value + elHeight) > (scrollTop + windowHeight) && offsetType.value == 'bottom' && !affix.value) {
          affix.value = true;
          state.styles = {
          ...
          emit('on-change', true);
      } else if ((elOffset.top + offsetBottom.value + elHeight) < (scrollTop + windowHeight) && offsetType.value == 'bottom' && affix.value) {
          affix.value = false;
          state.styles = {};
